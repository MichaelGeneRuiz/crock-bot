...
module.exports = {
    ...
    async execute(interaction) {
        ...
        .addFields(
            { name: 'Items', value: (items.length === 0) ? 'You have no items.' : items.map(i => `${i.item.name} ${i.item.icon}`).join('\n'), inline: true },
            { name: 'Quantity', value: (items.length === 0) ? 'N/A' : items.map(i => `${i.amount}`).join('\n'), inline: true },
        );
