...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (target.id === process.env.CREATOR_ID) {
            ...
            if (!storedBalances) { return interaction.editReply('No entries in the database, please fix.'); }
            ...
            .addFields(
                { name: 'Users', value: storedBalances.map(b => `${server.members.cache.get(b.user_id).user.tag}`).join('\n'), inline: true },
                { name: 'Balance', value: storedBalances.map(b => `${b.balance} CrockCoin`).join('\n'), inline: true },
            );
