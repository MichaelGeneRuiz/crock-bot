...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) =>
      option
        .setName("wager")
        .setDescription("Amount of currency to bet on the coin flip.")
        .setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    if (!user) {
      return interaction.reply(
        "You do not possess any currency yet. Please obtain some before continuing."
      );
    }
    ...
    // Currency checks
    if (userCurrency < wager) {
      return interaction.reply(
        `You cannot wager ${wager} CrockCoin, as you only have ${userCurrency} CrockCoin`
      );
    }
    if (wager <= 0) {
      return interaction.reply("Please wager an amount greater than zero.");
    }
    ...
    const resultEmbed = new EmbedBuilder(flipEmbed).spliceFields(2, 1, {
      name: "Results",
      value:
        result == 0 ? "The coin landed on heads." : "The coin landed on tails.",
    });
    ...
    if (result == 0 && interaction.options.getString("side") == "Heads") {
      ...
      const winningEmbed = new EmbedBuilder(resultEmbed).setFooter({
        text: `Congratulations! You won ${wager} CrockCoin! You now have ${interaction.client.currency.getBalance(
          target.id
        )} CrockCoin.`,
      });
      ...
      result == 1 &&
      interaction.options.getString("side") == "Tails"
    ) {
      interaction.client.currency.add(target.id, wager);
      const winningEmbed = new EmbedBuilder(resultEmbed).setFooter({
        text: `Congratulations! You won ${wager} CrockCoin! You now have ${interaction.client.currency.getBalance(
      ...
      interaction.client.currency.add(target.id, -wager);
      const winningEmbed = new EmbedBuilder(resultEmbed).setFooter({
        text: `Unlucky... You lost ${wager} CrockCoin... You now have ${interaction.client.currency.getBalance(
          target.id
        )} CrockCoin.`,
      });
      await interaction.editReply({ embeds: [winningEmbed] });
