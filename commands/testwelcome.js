...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (target.id === process.env.CREATOR_ID) {
            ...
            // Emoji Objects
            const userEmoji = interaction.guild.emojis.cache.find(emoji => emoji.name === process.env.USER_ROLE_EMOJI);
            const tempEmoji = interaction.guild.emojis.cache.find(emoji => emoji.name === process.env.TEMP_ROLE_EMOJI);

            // Send a message in the welcome channel
            const message = await interaction.client.channels.cache.get(process.env.WELCOME_CHANNEL_ID).send(`React to this message to get your roles!\nSelect ${userEmoji} to be a user.\nSelect ${tempEmoji} to be a member. (Requires a Password.)`);

            // React with the proper role emojis
            message.react(userEmoji)
