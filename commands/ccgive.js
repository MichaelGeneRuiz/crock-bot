...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option
        .setName("user")
        .setDescription("Which user to give currency to.")
        .setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    if (!user) {
      return interaction.reply(
        "You do not possess any currency yet. Please do so before continuing."
      );
    }
    ...
    if (giveAmount > currentAmount) {
      return interaction.reply(
        `You cannot give ${giveAmount} CrockCoin, as you only have ${currentAmount} CrockCoin`
      );
    }
    if (giveAmount <= 0) {
      return interaction.reply("Please give an amount greater than zero.");
    }
    ...
    return interaction.reply(
      `Successfully gave ${giveAmount} CrockCoin to ${
        giveTarget.username
      }. You now have ${interaction.client.currency.getBalance(
        interaction.user.id
      )} CrockCoin.`
    );
  },
