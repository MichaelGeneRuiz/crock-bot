...
const { NathanMessage, Users, UserItems, CurrencyShop } = require('../dbObjects.js');
...
module.exports = {
    ...
    data:  new SlashCommandBuilder()
        .setName('ccnathan')
        .setDescription('Changes the replyNathan message if you have a nathan token.')
        .addStringOption(option => option.setName('newmessage').setDescription('The message the bot will use.').setRequired(true)),
    async execute(interaction) {
        ...
        if (!user) { return interaction.reply('You do not possess any currency yet. Please do so before accessing this command.'); }
        ...
        if (!message) { await NathanMessage.create({ message_id: '0', message: 'test' }); }

        // Set the item to the Nathan Token from the shop
        const item = await CurrencyShop.findOne({ where: { name: { [Op.like]: 'Nathan Token' } } });
        ...
        if (nathantokens) {
            ...
            if (nathantokens.amount > 0) {
                ...
                // Reply to the user
                return interaction.reply(`The replyNathan message has been set to:\n ${newMessage}`);

            }
            else {
                // Tell the user they don't have enough tokens
                return interaction.reply('You need at least 1 Nathan Token to use this command.');
            }
        }
        else {
            // Tell the user they don't have enough tokens
