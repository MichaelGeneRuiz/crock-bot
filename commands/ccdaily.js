  ...
  async execute(interaction) {
    ...
    if (db_time !== null && timeout - (Date.now() - db_time) > 0) {
      ...
      return interaction.reply(
        `You have already claimed your daily prize, come back in ${timeLeft}.`
      );
    }
    // Otherwise
    else {
      // Reward the amount of CrockCoin
      await interaction.client.currency.add(
        interaction.user.id,
      ...
      return interaction.reply(
        `You claimed ${process.env.DAILY_REWARD} CrockCoin. You can claim again in 24 hours.`
      );
    }
  },
};
