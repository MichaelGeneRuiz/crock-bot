...
const DailyRewards = require('./models/DailyRewards.js')(sequelize, Sequelize.DataTypes);
const NathanMessage = require('./models/NathanMessage.js')(sequelize, Sequelize.DataTypes);
...
Reflect.defineProperty(Users.prototype, 'addItem', {
	value: async function addItem(item, amount) {
		...
		return UserItems.create({ user_id: this.user_id, item_id: item.id, amount: amount });
	},
});
...
Reflect.defineProperty(DailyRewards.prototype, 'setTime', {
	value: async function setTime(time) {
		const user = await DailyRewards.findOne({ where: { user_id: this.user_id } });
...
module.exports = { Users, CurrencyShop, UserItems, DailyRewards, NathanMessage };