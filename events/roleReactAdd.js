module.exports = {
  ...
  async execute(reaction, user) {
    ...
    const memberRole = reaction.message.guild.roles.cache.find(
      ...
      // If the temp role is selected
        ...
        if (
          reaction.message.guild.members.cache
            .get(user.id)
            .roles.cache.some((role) => role.name === process.env.MEMBER_ROLE)
        ) {
          // Send a message that they are already a member
          await user.client.channels.cache
        ...
        // Otherwise make them a temp member
          ...
          await user.client.channels.cache
            .get(process.env.SECRET_CHANNEL_ID)
            .send(
    ...
    // If the message is sent in the secret channel
      ...
      if (reaction.emoji.name === process.env.SECRET_EMOJI) {
        ...
        await user.client.channels.cache
          .get(process.env.SECRET_CHANNEL_ID)
          .send("Correct emoji. Adding Role")
          .then(() => {
            setTimeout(
              () =>
                reaction.message.guild.members.cache
      ...
      // Otherwise remove their temp role
        ...
        await user.client.channels.cache
          .get(process.env.SECRET_CHANNEL_ID)
          .send("Incorrect emoji. Removing you in a few seconds.")
          .then(() => {
            setTimeout(
              () =>
                reaction.message.guild.members.cache
