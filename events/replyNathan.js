...
module.exports = {
  ...
  async execute(message) {
    ...
    if (message.author.id !== process.env.NATHAN_ID) {
      return;
    }
    ...
    const replymessage = await NathanMessage.findOne({
      where: { message_id: "0" },
    });
    ...
    console.log(
      date.toLocaleString() +
        " | " +
        `replyNathan has been activated and will be available again in ${Math.ceil(
          cooldownTime * (166667 / 10000000000)
        )} minutes.`
    );
